...
//单个文件上传
function SingleUpload(repathImg, repath, uppath, maxFileSize, width, height, iswater) {

    var submitUrl = "../Tools/SingleUpload.ashx?ReFilePath=" + repath + "&UpFilePath=" + uppath + "&MaxFileSize=" + maxFileSize;
    //判断是否打水印
    if (arguments.length == 7) {
        submitUrl = "../Tools/SingleUpload.ashx?ReFilePath=" + repath + "&UpFilePath=" + uppath + "&MaxFileSize=" + maxFileSize + "&IsWater=" + iswater;
    }
    //开始提交
    $("#form1").ajaxSubmit({
        ...
        success: function (data, textStatus) {
            if (data.msg == 1) {
                var img = document.getElementById(repathImg);
                if (img) {
                    if (arguments.length >= 6)
                        img.src = "../Tools/Http_ImgLoad.ashx?w=" + width + "&h=" + height + "&gurl=" + data.msbox + "&timestamp=" + Date();
                    else
                        img.src = data.msbox + "?timestamp=" + Date();
                }
